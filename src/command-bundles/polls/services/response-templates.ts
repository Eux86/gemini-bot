...
const getNumberOfVotesForOption = (
  votes: IVote[],
  optionIndex: number,
): number => votes.filter((v) => v.optionIndex - 1 === optionIndex).length;

const getVotersForOption = (votes: IVote[], optionIndex: number) =>
  votes
    .filter((v) => v.optionIndex - 1 === optionIndex)
    .map((x) => x.userName)
    .sort()
    .join(', ');
...
const generateVoteInfo = (options: string[], votes: IVote[]) => {
  ...
  options.forEach((option, index) => {
    ...
    const listOfVoters =
      numberOfVotes > 0 ? ` (${getVotersForOption(votes, index)})` : '';

    returnString += `${
      index + 1
    }) ${option}\nVotes: ${numberOfVotes}${listOfVoters}${
